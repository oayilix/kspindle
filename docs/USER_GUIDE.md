...
### Step 2: Add SPI Framework dependencies
...
| `priority` | `Int` | `0` | Ordering priority. Higher values are returned first by `SpiLoader.load()` and sorted first in `SpiLoader.loadAll()`. / 排序优先级。数值越高的实现会被 `SpiLoader.load()` 优先返回，并在 `SpiLoader.loadAll()` 中优先排序。 |
| `name` | `String` | `""` | An optional logical name for this implementation, enabling retrieval by name via `SpiLoader.loadByName()`. / 此实现的可选逻辑名称，可通过 `SpiLoader.loadByName()` 按名称查询。 |
...
The main entry point for the SPI Framework. `SpiLoader` is a singleton object that discovers `ServiceIndexProvider` implementations (generated by the KSP processor) using `java.util.ServiceLoader`, populates the internal `ServiceRegistry`, and provides typed access to registered service implementations.

SPI 框架的主要入口点。`SpiLoader` 是一个单例对象，它通过 `java.util.ServiceLoader` 发现 `ServiceIndexProvider` 实现（由 KSP 处理器生成），填充内部的 `ServiceRegistry`，并提供对已注册服务实现的有类型访问。
...
// 使用自定义类加载器
SpiLoader.initialize(myCustomClassLoader)

// Idempotent -- safe to call multiple times
// 幂等 —— 可安全多次调用
SpiLoader.initialize()
SpiLoader.initialize() // no-op / 无操作
```
...
val greeting: GreetingService = SpiLoader.load(GreetingService::class.java)
...
```kotlin
val spanish: GreetingService? = SpiLoader.loadByName(GreetingService::class.java, "spanish")
if (spanish != null) {
    println(spanish.greet()) // "Hola"
}

val unknown: GreetingService? = SpiLoader.loadByName(GreetingService::class.java, "french")
println(unknown) // null -- no error thrown / null —— 不会抛出错误
```

---

#### `SpiLoader.isRegistered()`
...
```kotlin
if (SpiLoader.isRegistered(PaymentGateway::class.java)) {
    val gateway = SpiLoader.load(PaymentGateway::class.java)
}
```

---

#### `SpiLoader.getRegistry()`
...
将 SPI 框架重置为未初始化状态。清除所有注册并重置初始化标志。主要用于测试 —— 在 `@BeforeEach`/`@AfterEach` 中调用以防止测试污染。
...
The thread-safe registry that stores and retrieves service provider instances. Normally you interact with it through `SpiLoader`, but direct access is available for advanced scenarios via `SpiLoader.getRegistry()`.

线程安全的注册表，用于存储和检索服务提供者实例。通常通过 `SpiLoader` 与其交互，但在高级场景下可通过 `SpiLoader.getRegistry()` 直接访问。
...
Thrown by `SpiLoader.load()` when no implementation is registered for the requested service interface. The message includes the service class name and guidance on what to check.

当请求的服务接口没有注册实现时，由 `SpiLoader.load()` 抛出。消息中包含服务类名和检查建议。
...
Contract for generated code. The KSP processor generates one implementation of this interface per module that uses `@ServiceProvider` annotations. `SpiLoader` discovers these implementations at runtime via `java.util.ServiceLoader` and calls `initialize()` to populate the registry. End users never implement this interface directly.

生成代码的契约。KSP 处理器为每个使用 `@ServiceProvider` 注解的模块生成该接口的一个实现。`SpiLoader` 在运行时通过 `java.util.ServiceLoader` 发现这些实现，并调用 `initialize()` 填充注册表。最终用户不需要直接实现此接口。
...
1. **Higher priority values are returned first.** `SpiLoader.load()` returns the implementation with the highest priority. `SpiLoader.loadAll()` sorts descending by priority.
   **数值越高的优先级越先返回。** `SpiLoader.load()` 返回优先级最高的实现。`SpiLoader.loadAll()` 按优先级降序排序。
...
The `name` parameter on `@ServiceProvider` gives each implementation a logical identifier that can be used for targeted retrieval via `SpiLoader.loadByName()`.

`@ServiceProvider` 上的 `name` 参数为每个实现提供了一个逻辑标识符，可通过 `SpiLoader.loadByName()` 进行定向检索。
...
- `SpiLoader.loadByName()` returns `null` (not throws) when no matching name is found. Always check the result for `null`.
- `SpiLoader.loadByName()` 在未找到匹配名称时返回 `null`（而不是抛出异常）。始终检查结果是否为 `null`。
...
fun getTranslator(provider: String): Translator? =
    SpiLoader.loadByName(Translator::class.java, provider)
```
...
- Instance is created on the first call to `getInstance()` (the first time someone calls `SpiLoader.load()`, `loadAll()`, or `loadByName()` on this service class).
- 实例在首次调用 `getInstance()` 时创建（即有人首次在此服务类上调用 `SpiLoader.load()`、`loadAll()` 或 `loadByName()` 时）。
...
- Instance is created during registration, before `SpiLoader.initialize()` completes.
- 实例会在注册期间创建，也就是在 `SpiLoader.initialize()` 完成之前创建。
...
// Created as soon as registry entries are populated during SpiLoader.initialize()
// 在 SpiLoader.initialize() 填充注册表条目时立即创建
@ServiceProvider(service = HealthCheck::class, lazy = false)
...
The SPI Framework is designed for concurrent access from the ground up.

SPI 框架从一开始就设计为支持并发访问。
...
|---|---|---|
| `SpiLoader.initialize()` | Double-checked locking with `@Volatile` flag and `synchronized` block / 使用 `@Volatile` 标志和 `synchronized` 块的双重检查锁定 | Yes / 是 |
| `SpiLoader.load()` / `loadAll()` / `loadByName()` | Delegates to `ServiceRegistry` (read-only after initialization) / 委托给 `ServiceRegistry`（初始化后只读） | Yes / 是 |
| `SpiLoader.reset()` | `synchronized(initLock)` | Yes / 是 |
...
1. **Call `SpiLoader.initialize()` once at application startup** from a safe initialization point (e.g., `Application.onCreate()` in Android, `main()` in a JVM application). After initialization, all read operations are lock-free on `ConcurrentHashMap`.
   **在应用启动时调用一次 `SpiLoader.initialize()`**，从安全的初始化点调用（例如 Android 中的 `Application.onCreate()`，JVM 应用中的 `main()`）。初始化后，所有读取操作在 `ConcurrentHashMap` 上均为无锁操作。
...
3. **`SpiLoader` is safe to use as a singleton.** It is itself an `object` (Kotlin singleton), and the `ServiceRegistry` is a single instance held inside it.
   **`SpiLoader` 可以作为单例安全使用。** 它本身是一个 `object`（Kotlin 单例），`ServiceRegistry` 是其内部持有的单个实例。
...
**Cause:** `SpiLoader.load()` was called but no `@ServiceProvider`-annotated class was found for the requested service interface at compile time, or the KSP processor was not applied to the module containing the annotated classes.

**原因：** 调用了 `SpiLoader.load()`，但在编译时未找到请求的服务接口的 `@ServiceProvider` 注解类，或者 KSP 处理器未应用于包含注解类的模块。
...
2. **Verify the KSP processor.** Check that `ksp("...spi-compiler:...")` is applied in the build.gradle.kts of the module where the `@ServiceProvider` annotations live (not just the consuming module).
   **验证 KSP 处理器。** 检查 `ksp("...spi-compiler:...")` 是否已添加到 `@ServiceProvider` 注解所在模块的 build.gradle.kts 中（而不仅仅是消费模块）。
...
**Cause:** The KSP processor (`spi-compiler`) did not run on the module containing `@ServiceProvider` annotations.

**原因：** KSP 处理器（`spi-compiler`）未在包含 `@ServiceProvider` 注解的模块上运行。
...
2. Ensure `ksp(project(":spi-compiler"))` or `ksp("com.spi.framework:spi-compiler:VERSION")` is in the module's `dependencies` block.
   确保模块的 `dependencies` 块中有 `ksp(project(":spi-compiler"))` 或 `ksp("com.spi.framework:spi-compiler:VERSION")`。
...
- 添加无参构造函数，或者
- Register the service programmatically via `SpiLoader.getRegistry().register()` with a pre-built instance (future feature).
- 通过 `SpiLoader.getRegistry().register()` 使用预构建实例以编程方式注册服务（未来特性）。
...
| Java ServiceLoader | SPI Framework / SPI 框架 |
|---|---|
| `META-INF/services/` files / 文件 | Generated automatically by KSP processor / 由 KSP 处理器自动生成 |
| `ServiceLoader.load(Class)` | `SpiLoader.load(Class)` |
| `ServiceLoader.iterator()` | `SpiLoader.loadAll(Class)` |
| No ordering / 无排序 | Priority-based ordering / 基于优先级的排序 |
| No named lookups / 无命名查询 | `SpiLoader.loadByName()` |
