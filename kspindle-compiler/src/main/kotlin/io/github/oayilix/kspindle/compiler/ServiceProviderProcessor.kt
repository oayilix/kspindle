 ...
 * 1. A [io.github.oayilix.kspindle.runtime.ServiceIndexProvider] implementation that registers
 *    all discovered services into the [io.github.oayilix.kspindle.runtime.ServiceRegistry].
 *    一个 [ServiceIndexProvider] 实现，将所有发现的服务注册到 [ServiceRegistry]。
 * 2. A META-INF/services file for [io.github.oayilix.kspindle.runtime.ServiceIndexProvider]
...
class ServiceProviderProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        if (annotatedSymbols.isEmpty()) {
            logger.info("[KSPindle Processor] No @ServiceProvider annotations found")
            return emptyList()
        }

        logger.info("[KSPindle Processor] Found ${annotatedSymbols.size} @ServiceProvider-annotated class(es)")
        ...
        for (provider in validProviders.filter { it.name.isNotEmpty() }) {
            ...
            if (!seen.add(key)) {
                logger.warn(
                    "[KSPindle Processor] Duplicate @ServiceProvider registration found: " +
        ...
        logger.info("[KSPindle Processor] Provider summary by service interface:")
        for ((serviceName, impls) in groupedByService) {
            logger.info(
                "[KSPindle Processor]   ${impls.size} provider(s) for $serviceName: " +
                impls.joinToString(", ") { it.implQualifiedName }
            )
        }

        if (validProviders.isEmpty()) {
            logger.warn("[KSPindle Processor] No valid @ServiceProvider declarations to process")
            return emptyList()
        }
        ...
        logger.info("[KSPindle Processor] Generated ServiceIndex_$suffix with ${validProviders.size} registration(s)")
    ...
    private fun processAnnotation(
        ...
        for (annotation in annotations) {
            ...
            val usesFactory = factoryType != null &&
                factoryType.declaration.qualifiedName?.asString() != "io.github.oayilix.kspindle.annotations.ServiceFactory.None"
    ...
    private fun generateServiceIndexClass(
        ...
        val packageName = "io.github.oayilix.kspindle.generated"
        ...
        val sourceCode = """
            |package $packageName
            |
            |import io.github.oayilix.kspindle.runtime.ServiceIndexProvider
            |import io.github.oayilix.kspindle.runtime.ServiceRegistry
            |$allPackages
            |
            |/**
            | * Auto-generated by KSPindle KSP Processor.
            | * 由 KSPindle KSP 处理器自动生成。
            | *
    ...
    private fun generateServiceDescriptorFile(suffix: String) {
        val className = "ServiceIndex_$suffix"
        val serviceLine = "io.github.oayilix.kspindle.generated.$className"

        try {
            val file = codeGenerator.createNewFileByPath(
                dependencies = Dependencies(aggregating = true),
                path = "META-INF/services/io.github.oayilix.kspindle.runtime.ServiceIndexProvider",
                extensionName = ""
            )
            OutputStreamWriter(file).use { writer ->
